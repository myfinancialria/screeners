...
def build_html(top, total, valid, when):
    ...
    for s in SETUP_ORDER:
        rows = [a for a in top if a["bull_setup"] == s]
        if not rows:
            continue
        label, color = SETUP_LABEL[s]
        sections += (
            f"<h2 style='border-color:{color}'>{label} <span style='color:#6b7686'>"
            f"({len(rows)})</span></h2>"
...
{sections}
<p class="foot">Auto-generated by a FYERS-powered scanner. Setups are ranked
early-first (pre-breakout → breakout → pullback). <b>For educational use only —
not investment advice.</b> Verify charts and do your own research before trading.</p>
</div></body></html>"""
